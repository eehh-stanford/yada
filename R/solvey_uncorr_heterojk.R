#' @title Do a maximum likelihood fit for theta_y for the uncorrelated, heteroskedastic case with separate kappa's for each variable
#'
...
#' @export
solvey_uncorr_heterojk <- function(x,Y,hp,verbose=F) {
 ...
 if(J > 0) {
  ...
  for(j in 1:J) {
    ...
    xj <- xj[!is.na(vj)]
    vj <- vj[!is.na(vj)]
    hp_j <- list(paramModel='powLawOrdHetero')
    hp_j$J <- 1
    hp_j$M <- length(unique(vj)) - 1
    ordFit <- fitPowLawOrd(xj,vj,hetero=T)
    ordFitList[[j]] <- ordFit
 ...
 if(K > 0) {
  ...
  for(k in 1:K) {
    ...
    xk <- xk[!is.na(wk)]
    wk <- wk[!is.na(wk)]
    contFit <- fitPowLaw(xk,wk,hetero=T)
  ...
  theta_y_list <- list(paramModel='powLawMixUncorrHeterojk')
  if(J > 0) {
