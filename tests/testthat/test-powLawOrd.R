...
expect_equal(
  powLawOrdSigma(5,th_v_hetero,'sd_x',F),
  0.01*(1+5*0.02)
)

expect_equal(
  powLawOrdSigma(5,th_v_bar_hetero,'sd_x',T),
  0.01*(1+5*0.02)
)
...
expect_error(
  sim_hetero <- simPowLawOrd(N,th_x,th_v_hetero,hetSpec='sd_x'),
  NA
)
...
expect_error(
  fit_hetero <- fitPowLawOrd(sim_hetero$x,sim_hetero$v,hetSpec='sd_x'),
  NA
)
